...
class ConversionWorker:
    ...
    def process_task(self, task_id: str) -> bool:
        ...
        if not document:
            logger.error(f"文档不存在: {task.document_path}")
        ...
        try:
            ...
            # 确定目标路径
            target_filename = f"{document.path}.pdf"
...
def process_conversion_task_async(task_id: str, db_session_factory):
    """异步处理转换任务（用于Celery）"""
...
def create_conversion_task_for_document(
    ...
    """为文档创建转换任务（使用文档路径）"""
    # 获取文档
    document = db.query(Document).filter(Document.path == document_path).first()
    if not document:
    ...
    existing_task = db.query(ConversionTask).filter(
        ConversionTask.document_path == document_path,
        ConversionTask.status.in_([TaskStatus.QUEUED, TaskStatus.PROCESSING])
    ).first()
    
    if existing_task:
        logger.warning(f"文档已有活跃的转换任务: {document_path} ({existing_task.status})")
    ...
    logger.info(f"创建转换任务: {task.id} 为文档 {document_path}")
    return task