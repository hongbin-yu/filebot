#!/usr/bin/env python3
"""
修复版：将COLD_INDEXES CSV转换为FileBot JSON配置
正确处理DATEFORMAT和其他字符串值
"""
    ...
    with open(csv_file, 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader):
            try:
                ...
                # 处理可能为空的值
                left_offset_str = row.get('LEFT_OFFSET', '').strip()
                left_offset = int(left_offset_str) if left_offset_str and left_offset_str != 'NULL' else 0
                ...
                if literal and literal != 'NULL' and literal != ' ':
                    field_name = f"{literal.lower()}_{coln}"
                else:
                ...
                if dateformat and dateformat != 'NULL':
                    ...
                    # 如果是日期格式，设置字段类型
                    if dateformat.lower() in ['mdy', 'yyyymmdd'] or 'm' in dateformat.lower() or 'y' in dateformat.lower():
...
def main():
    if len(sys.argv) != 3:
        ...
        print(f"示例: {sys.argv[0]} cold_indexes.csv cold_indexes_config_fixed.json")
        sys.exit(1)
