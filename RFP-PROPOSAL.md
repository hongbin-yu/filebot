...
> 
> Every capability described in this document can be **demonstrated live, inspected, and validated** before any commitment is made.
> 
> **Yuhong Web Inc.** (incorporated July 2017) has been operating since 2017. FileBot — our asset management engine — is a mature system with **5 active users across US-based organizations**. WebBot, our content management layer, is the latest AI-augmented addition to an established platform.
> 
> **We do not ask for trust. We ask for verification.**
> 
> Live systems running today:
...
- **Verified**: local Phi3 outperforms OpenAI for Canada.ca content Q&A — see [§3.3](#33-semantic-search--ai-qa)
...
We offer a fundamentally different approach — not a proposal for a future system, but a **working, verifiable platform** built today:

> **Designed by 20 years of Canada.ca experience. Developed by an AI-empowered team. Verifiable on demand. Operationally proven.**

Our platform consists of two integrated systems — **WebBot** (intranet content management, AI-augmented) and **FileBot** (mature asset management & delivery system, under continuous development since 2017 with active deployments) — that together provide a complete, secure, and cost-effective content management platform.
...
| **Small team, AI-empowered** | 20 years of domain expertise + AI tooling = focused, efficient team |
...
| **Publish** | AEM Publish instance + replication | Static file servers (existing infra) |

**Why ours is lighter:**
- No JCR means no AEM instance to deploy per environment
- Configuration is code (git-tracked), not JCR nodes
- Each environment is a copy of the same WebBot + FileBot — no special licensing needed
- Promotion between environments is git-based (merge → deploy → test), not replication-based
- **Result**: The same 4-tier quality assurance, but deployable by a single person instead of a team

### 2.3 AI-Assisted System Maintenance

**Current problem — fully manual maintenance:**
- Server updates, security patches, configuration changes: all done by a human
- Each upgrade carries risk of human error (wrong config, missed step, incorrect version)
- Dependency on specific individuals who "know how things work"
- No audit trail for system changes

**Our approach — human monitors, AI executes:**
...
|---|---|---|
| Security patching | Human finds, tests, deploys | AI scans, plans, executes with human approval |
| Configuration changes | Human edits files, risks typos | AI follows config-as-code, validates syntax |
| OS / dependency updates | Human schedules & risks missing | AI monitors CVEs, proposes & applies updates |
| Backup verification | Human checks logs | AI verifies backups and reports status |
| Incident response | Human on-call, slow diagnosis | AI triages, suggests root cause, common fixes auto-applied |

**Key benefit: fewer human errors.**
- AI doesn't make typographical mistakes
- AI doesn't miss a step in a 20-step procedure
- AI documents every action for audit
- Human remains in control — reviews and approves all critical changes
- Routine maintenance now takes **minutes instead of hours**
...
- **Operational independence.** Each system can be upgraded, patched, or scaled independently. AI pipeline upgrades don't require editorial downtime.
...
> **Design opinion:** This dual-write architecture is the right trade-off. A single monolithic database sounds cleaner on paper but introduces coupling between editorial and AI workloads. Two databases with a reliable sync layer gives us the best of both: independent scaling and operational isolation. The "extra complexity" of the dual write is fully contained in a single function in the import router — the rest of the system never needs to know there are two databases.
...
- New component: **hours, not months**
...
> **Opinion:** The bookmarklet import was one of the most practical additions we built. Canada.ca editors deal with hundreds of existing pages every day. A tool that lets them grab any page from the live site and bring it into the editing environment in one click — without copying files, mapping components, or waiting — is the difference between "this system is useful someday" and "this system saves me 30 minutes right now."
...
**What this means for the government ecosystem:**

- Existing contractors and developers **can continue building value-added services** — just using modern API calls instead of AEM Java components
- A developer can build a departmental dashboard, a mobile app, or an automated workflow with **any programming language** — Python, JavaScript, Go, PowerShell
- New features don't require AEM expertise — just standard API skills
- **Integration with GC systems** (pay, benefits, immigration) is straightforward — REST APIs with JWT auth
- **Third-party vendors are not locked out** — our API is open and well-documented

> "Adopting our system doesn't mean less work for the ecosystem. It means more valuable work — building modern, API-driven government services instead of maintaining Java components."
...
| **Intranet Security** | Requires separate AEM Author license | Built-in (WebBot on intranet) |
...
The single biggest fear among government departments is migration. AEM's migration experience — costly projects with limited progress — has created deep institutional skepticism.
...
| Department fears | No forced migration, opt-in only |
...
- Government has no financial risk — if the system doesn't deliver, walk away
...
|---|---|---|
| WYSIWYG HTML editor | ✅ Live | WebBot (intranet) |
| Mustache template engine | ✅ Live | WebBot + FileBot |
| Content management (CRUD, folders, publish) | ✅ Live | WebBot |
| Semantic vector search (pgvector) — 29,304 indexed pages | ✅ Live | FileBot |
| AI Q&A (OpenAI + Ollama, provider toggle) | ✅ Live | FileBot (cloud) |
| Website crawling & sitemap import | ✅ Live | FileBot |
| AI content classification | ✅ Live | FileBot |
| Zero-transformation content import | ✅ Live | FileBot |
| Multi-format file conversion (PCL→PDF, TIFF→JPG, TXT) | ✅ Live | FileBot |
| Version management & publishing | ✅ Live | WebBot + FileBot |
| User & permission management | ✅ Live | WebBot |
| Bilingual content (EN/FR mirror) | ✅ Live | WebBot |
| Full REST API for extensibility | ✅ Live | All endpoints |
| Data export / import | ✅ Live | FileBot |
| Bookmarklet one-click import (dual-write to both databases) | ✅ Live | FileBot + WebBot |
| Dual-database architecture (SQLite editorial + PostgreSQL assets/AI) | ✅ Live | WebBot + FileBot |
| Skip-if-exists deduplication | ✅ Live | FileBot + WebBot |
| Auto path transformation during import | ✅ Live | FileBot (+ WebBot) |
| Cross-system token-authenticated API sync | ✅ Live | FileBot → WebBot |
| Analytics & tracking | ✅ Live | Publish server (intranet) |
| Static publish pipeline (CDN-ready) | ✅ Live | Publish server (intranet) |

### 10.1 Why This Matters

> **Most CMS RFPs compare proposals against proposals. Ours is a working system ready for evaluation.**

We are not asking the government to trust our promises. We are asking them to verify our working system — side by side with an existing AEM deployment — and evaluate the results.
...
> "Pick us for the CMS today. Tomorrow's AI search, translation, and Q&A are already here."
...
### 11.2 Why Canada.ca Team Should Not Fear Us

Our strategy is partnership, not replacement:

- We need their content expertise — it's irreplaceable
- They gain freedom from AEM lock-in
- Components go from 6 months to hours — they can do more meaningful work
- The existing CDN, load balancer, and publish infrastructure stays the same
- No job loss. Only job enhancement.
